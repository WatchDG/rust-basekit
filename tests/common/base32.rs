...
pub fn roundtrip(original: &[u8]) {
    let encoded = Vec::<u8>::from(encode(&enc_config(Some(b'=')), original));
    let decoded = Vec::<u8>::from(decode(&dec_config(Some(b'=')), &encoded).unwrap());
...
pub fn roundtrip_into(original: &[u8]) {
    ...
    let actual_enc_len = encode_into(&enc_config(Some(b'=')), &mut enc_dst, original).unwrap();
    assert_eq!(actual_enc_len, enc_len);

    let mut dec_dst = vec![0u8; original.len()];
    let actual_dec_len = decode_into(
        &dec_config(Some(b'=')),
        &mut dec_dst,
...
pub fn roundtrip_no_padding(original: &[u8]) {
    let encoded = Vec::<u8>::from(encode(&enc_config(None), original));
    let decoded = Vec::<u8>::from(decode(&dec_config(None), &encoded).unwrap());
    assert_eq!(
        decoded, original,
...
pub fn roundtrip_no_padding_into(original: &[u8]) {
    ...
    let mut enc_dst = vec![0u8; enc_len];
    let actual_enc_len = encode_into(&enc_config(None), &mut enc_dst, original).unwrap();
    assert_eq!(actual_enc_len, enc_len);

    let mut dec_dst = vec![0u8; original.len()];
    let actual_dec_len =
        decode_into(&dec_config(None), &mut dec_dst, &enc_dst[..actual_enc_len]).unwrap();
    assert_eq!(
        &dec_dst[..actual_dec_len],
        original,
...
pub fn exact_decode_into_no_padding(data: &[u8]) {
    ...
    let encoded = Vec::<u8>::from(encode(&enc, data));
    let expected = Vec::<u8>::from(decode(&dec, &encoded).unwrap());

    let mut dst = vec![0u8; expected.len()];
    let len = decode_into(&dec, &mut dst, &encoded).unwrap();
