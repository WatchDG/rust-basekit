use basekit::base64::{
    ALPHABET_BASE64, Base64DecodeConfig, Base64EncodeConfig, decode, decode_into, encode,
    encode_into, DECODE_TABLE_BASE64,
};
...
fn roundtrip_into(original: &[u8]) {
    ...
    let actual_decoded_len = decode_into(&dec_config, &mut decoded_dst, &encoded_dst[..actual_encoded_len]).unwrap();
