...
use basekit::base32::{ALPHABET_BASE32, Base32EncodeConfig, Base32Error, encode32_into};
...
fn test_encode_into_single_byte() {
    let config = create_config();
    let mut dst = [0u8; 8];
    let len = encode32_into(&config, &mut dst, &[102]).unwrap();
    assert_eq!(len, 8);
    assert_eq!(&dst, b"MY======");
}
...
fn test_no_write_beyond_returned_length() {
    ...
    let len = encode32_into(&config, &mut dst, b"f").unwrap();
