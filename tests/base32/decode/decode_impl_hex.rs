use basekit::base32::{Base32DecodeConfig, Base32Error, DECODE_TABLE_BASE32_HEX, decode32};
...
fn test_invalid_character() {
    let config = create_config();
    let result = decode32(&config, b"0000!");
...
fn test_invalid_character_high_byte() {
    let config = create_config();
    let result = decode32(&config, b"00\xFF00==");
    assert!(matches!(
        result,
...
fn test_base32hex_encodes_zeros_differently() {
    ...
    let result = encode32(&encode_config, &[0]);
...
fn test_base32hex_decodes_zeros_correctly() {
    let config = create_config();
    let result = decode32(&config, b"00======").unwrap();
